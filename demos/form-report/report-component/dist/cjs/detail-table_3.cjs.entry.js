...
const ReportDetailTable = class {
    ...
    render() {
        return (index.h(index.Host, { key: '793814a918a803c0d38341450e369dce9ab63dee' }, index.h("calcite-table", { key: '079fab8aa1e4ae3f8fabc86cac081f0ea3b5d063', caption: "job details", striped: true }, index.h("calcite-table-row", { key: 'dc681f041274bc9d806885c95cbb5515c5214520' }, index.h("calcite-table-cell", { key: '27d51f0392542859dd1a3bb9087a1cb9602ecb5b' }, this.langTasks.detailJobID), index.h("calcite-table-cell", { key: 'ea8c844cefe02794f98d59ce306309bfcb09af97' }, this.job.jobId)), index.h("calcite-table-row", { key: '624de8596c065cee660065e81ac394190ed1c3af' }, index.h("calcite-table-cell", { key: '03abeea4269ef764429126abc72ada966409dc84' }, this.langTasks.detailStatus), index.h("calcite-table-cell", { key: '2e09e7f696a9b06a6c0b129e23c972ca165d5efb' }, index.h("div", { key: '3eb12fa114170e551bbb34cd3173a2dfe70ae89b' }, index.h("span", { key: '6519b2c268c6711d808580ff28a754f8324f491a' }, this.statusI18nConfig[this.job.jobStatus]), this.job.jobStatus == 'esriJobPartialSucceeded' && this.job.resultInfo.totalCount ?
            index.h("span", null, this.langTasks.jobStatusPartialSucceeded.replace('${$completedPercent}', `${this.getPercentage(1 - this.job.resultInfo.failedObjectIds.length / this.job.resultInfo.totalCount)}`)) : ''))), index.h("calcite-table-row", { key: 'b88ad596a3faa322e199a06ba5c81d392a68cb26' }, index.h("calcite-table-cell", { key: 'cff3c12af400d0d56b97eee26ae6c62ba4297c97' }, this.langTasks.detailStatusMsg), index.h("calcite-table-cell", { key: '7aa53b2f8950c58e8e7ea437ce19306f4e5efe6f' }, index.h("div", { key: 'a40d72ca0c00b7069e9f257c08ca1c6b33fc97fd', innerHTML: this.errorMsg, title: this.errorMsg }))), index.h("calcite-table-row", { key: 'f0128bf112b9583606b6c7e37077372e14a7504e' }, index.h("calcite-table-cell", { key: '60c32b9a1b5c8afe9b545f504d2435276a652479' }, this.langTasks.detailFailedOIDs, " ", (this.job.resultInfo && this.job.resultInfo.failedObjectIds && this.job.resultInfo.failedObjectIds.length ? ' (' + this.job.resultInfo.failedObjectIds.length + ') ' : '')), index.h("calcite-table-cell", { key: '3b9641affd8a60f44e626fd269eb59be43b8d5dd' }, this.job.resultInfo && this.job.resultInfo.failedObjectIds && this.job.resultInfo.failedObjectIds.length ?
            index.h("div", null, index.h("p", { class: "max-tow-lines", title: this.job.resultInfo.failedObjectIds.join(',') }, this.job.resultInfo.failedObjectIds.join(',')), this.job.resultInfo.details && this.job.resultInfo.details.length ?
                index.h("p", { class: "more-details" }, index.h("a", { onClick: () => this.errorDetailModalOpen() }, this.langTasks.detailMoreDetails)) : '')
            : '')), index.h("calcite-table-row", { key: '1541ad8f6290adea97b5725eb7e5e6a260ca1867' }, index.h("calcite-table-cell", { key: 'a8e403c7030e9320372336ff99cc1ab5ac86d1d5' }, this.langTasks.detailSucceededOIDs, " ", this.job.resultInfo && this.job.resultInfo.succeededObjectIds && this.job.resultInfo.succeededObjectIds.length ? this.job.resultInfo.succeededObjectIds.length : ''), index.h("calcite-table-cell", { key: '5aabd258902f4b6a6f0ca7e5d8c25b67c2a81049' }, this.job.resultInfo && this.job.resultInfo.succeededObjectIds && this.job.resultInfo.succeededObjectIds.length ?
            index.h("div", null, index.h("p", { class: "max-tow-lines", title: this.job.resultInfo.succeededObjectIds.join(',') }, this.job.resultInfo.succeededObjectIds.join(',')), index.h("p", { class: "more-details" }, index.h("a", { onClick: () => this.succeedDetailModalOpen() }, this.langTasks.detailMoreDetails))) : ''))), index.h("error-detail-modal", { key: 'e206fadb79ea76932da05b48b4910c759465626e', class: "error-modal", job: this.job }), index.h("success-detail-modal", { key: '2d40fe7cda776b6a5ff63148a55a7c1b6feeb4e2', class: "success-modal", job: this.job }), index.h("slot", { key: '7a4282727b8de61474110961712ba1a22cb4caff' })));
    }
...
const ReportIcon = class {
    ...
    fetchIcon() {
        const utils = util_service.UtilService.getService();
        return Promise.resolve(true)
            .then(() => {
            if (utils.getSvgCache(this.icon)) {
                return utils.getSvgCache(this.icon);
            }
            return fetch(index.getAssetPath(`./assets/icons/${this.icon}.svg`))
                .then((resp) => {
                if (!resp.ok) {
    ...
    render() {
        return (index.h(index.Host, { key: 'bfda6b93ffc35d676f665226e95b4cb92dc45089' }, index.h("i", { key: '3f0f5a2e470f746a6af14fac54108c4366f33cc5', innerHTML: this.pathData }), index.h("slot", { key: '8c22b8b576cdc451d11cd323fef745bdc7757ce8' })));
    }
};
...
const ReportTaskInfo = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.jobRemoved = index.createEvent(this, "jobRemoved", 7);
        this.isRTL = false;
        this.maxFailedOIDLength = 120;
        this.utils = util_service.UtilService.getService();
    ...
    updateJobStatusString() {
        ...
        if (['esriJobWaiting', 'esriJobSubmitted', 'esriJobExecuting'].indexOf(this.job.jobStatus) > -1) {
            if (this.job.jobStatus == 'esriJobExecuting' && this.job.jobStatusInfo && this.detailedStatus) {
                ...
                this.detailedStatusEle = index.h("span", null, i18nStr.replace('${$completedPercent}', `${progressPercent}`));
            }
        }
        else {
            if (this.job.jobStatus == 'esriJobExecuting' && (!this.job.jobStatusInfo || !this.detailedStatus)) {
                this.detailedStatusEle = index.h("span", null, this.langTasks.jobStatusPrinting);
            }
            else {
                this.detailedStatusEle = index.h("span", null, this.statusI18nConfig[this.job.jobStatus]);
            }
        }
    }
    ...
    render() {
        return (index.h(index.Host, { key: 'b081ef135cdbec5d1c194b69887337fc9c5ea959' }, index.h("calcite-card", { key: '707b5cc07e84a124276c2ba29abb70f9a0106962', class: `root ${!this.job ? 'hidden' : ''} ` }, this.job ?
            index.h("div", { class: `job-info ${this.showDetail ? 'hasDetail' : ''}` }, index.h("i", { class: "status-icon" }, ['esriJobWaiting', 'esriJobSubmitted', 'esriJobExecuting'].indexOf(this.job.jobStatus) >= 0 ? index.h("calcite-icon", { scale: "s", icon: "hourglass-active", "text-label": this.statusI18nConfig[this.job.jobStatus] }) : '', this.job.jobStatus == 'esriJobSucceeded' ? index.h("report-icon", { icon: "success" }) : '', this.job.jobStatus == 'esriJobPartialSucceeded' ? index.h("report-icon", { icon: "success-caveat" }) : '', this.job.jobStatus == 'esriJobStatusCancelled' ? index.h("calcite-icon", { scale: "s", icon: "circle-disallowed", "text-label": this.langTasks.jobStatusCanceled }) : '', this.job.jobStatus == 'esriJobFailed' ? index.h("calcite-icon", { scale: "s", icon: "x-circle", "text-label": this.langTasks.jobStatusFailed }) : ''), index.h("div", { class: "detail" }, index.h("p", { class: "header" }, this.job.resultInfo ? this.job.resultInfo.title : '', this.job.inputInfo && this.job.inputInfo.parameters && this.job.inputInfo.parameters.outputFormat == 'pdf' ?
                index.h("calcite-icon", { scale: "s", icon: this.isRTL ? '' : 'file-pdf', "text-label": "" })
                : index.h("calcite-icon", { scale: "s", icon: this.isRTL ? '' : 'file-word', "text-label": "" })), !(this.isWaitingToConfirmRemove) ?
                index.h("p", { class: "middle-line" }, index.h("span", { class: "records" }, this.langTasks.records, this.job.resultInfo && this.job.resultInfo.totalCount ? this.job.resultInfo.totalCount : ''), index.h("span", { class: "startTime", title: this.utils.formatDateTime(this.job.submitted, 'datetime') }, this.langTasks.started, this.utils.formatDateTime(this.job.submitted, 'time'))) : '', !(this.isWaitingToConfirmRemove) ?
                index.h("p", { class: "status" }, ['esriJobWaiting', 'esriJobSubmitted', 'esriJobExecuting'].indexOf(this.job.jobStatus) > -1 ?
                    index.h("span", null, this.langTasks.status, " ", this.detailedStatusEle, " ")
                    :
                        index.h(index.Fragment, null, ['esriJobWaiting', 'esriJobSubmitted', 'esriJobExecuting'].indexOf(this.job.jobStatus) == -1 ?
                            index.h("span", null, this.langTasks.duration, " ", this.getDuration(this.job.submitted, this.job.completed, this.job.cancelled)) : ''), this.job.jobStatus == 'esriJobPartialSucceeded' || this.job.jobStatus == 'esriJobFailed' ?
                    index.h(index.Fragment, null, !this.showDetail ?
                        index.h("a", { class: "job-detail-link", onClick: () => this.showDetailHandler() }, this.langCommon.showDetails)
                        : '', this.showDetail == 'pending' ?
                        index.h("a", { class: "job-detail-link job-detail-pending" }, this.langCommon.loading, index.h("i", { class: "icon icon-spin disabled" }, index.h("calcite-loader", { label: "", inline: "true" }))) : '', this.showDetail == 'done' ?
                        index.h("a", { class: "job-detail-link", onClick: () => this.hideDetail() }, this.langCommon.hideDetails)
                        : '')
                    : '') :
                index.h(index.Fragment, null, this.isWaitingToConfirmRemove ?
                    index.h(index.Fragment, null, index.h("p", null, this.langTasks.removeJobConfirm), index.h("p", null, index.h("a", { class: "confirm-delete-anchor", onClick: () => this.executeRemoveJob(this.job) }, this.langCommon.yes), index.h("a", { class: "cancel-delete-anchor", onClick: () => this.cancelRemoveJob() }, this.langCommon.no))) : ''), !this.isWaitingToConfirmRemove ?
                index.h("div", { class: "icon-area dropdown" }, this.job.jobStatus == 'esriJobSucceeded' && this.job.urlExpired ?
                    index.h("a", null, index.h("calcite-icon", { scale: "s", icon: "download", "text-label": "", disabled: true, id: this.jobId + '-disabled-btn' }), index.h("calcite-tooltip", { label: this.langTasks.downloadDisabledTip, "reference-element": this.jobId + '-disabled-btn' }, index.h("span", null, this.langTasks.downloadDisabledTip))) : '', (this.job.jobStatus == 'esriJobSucceeded' || this.job.jobStatus == 'esriJobPartialSucceeded') && this.job.url && !this.job.urlExpired && !this.job.multipleFiles ?
                    index.h("a", { href: this.job.url, title: this.langCommon.download, download: true, target: this.job.inputInfo && this.job.inputInfo.parameters && this.job.inputInfo.parameters.outputFormat == 'pdf' ? '_blank' : '' }, index.h("i", { class: "download-icon" }, index.h("calcite-icon", { scale: "s", icon: "download", "text-label": "" })))
                    ...
                    index.h("a", { class: "dropdown-toggle" }, index.h("calcite-dropdown", { width: "m", placement: "bottom-end", type: "click" }, index.h("i", { slot: "trigger", class: "download-icon", "data-toggle": "dropdown", role: "button", "aria-haspopup": "true", "aria-expanded": "false" }, index.h("calcite-icon", { scale: "s", icon: "download", "text-label": "" })), this.job.resultInfo.resultFiles.map((file) => {
                        return index.h("calcite-dropdown-item", null, index.h("div", { class: "menu-item" }, index.h("a", { href: file.url, title: file.name }, index.h("span", { class: "left-text" }, file.name || '')), index.h("span", { class: "right-text" }, file.size || '')));
                    ...
                    index.h("a", { class: this.isCanceling ? 'isloading' : '', title: (this.isCanceling) ? '' : `${this.langTasks.cancelJobTip}` }, !(this.isCanceling) ?
                        index.h("i", { class: "download-icon", onClick: () => this.startCancelJob(this.job) }, index.h("calcite-icon", { scale: "s", icon: "circle-stop", "text-label": "" }))
                        :
                            index.h("i", { class: "download-icon spinner" }, index.h("calcite-loader", { label: this.langTasks.jobStatusCanceled, inline: "true" })))
                    : '', ['esriJobSucceeded', 'esriJobPartialSucceeded', 'esriJobFailed', 'esriJobStatusCancelled'].indexOf(this.job.jobStatus) >= 0 ?
                    index.h("a", { class: this.isRemoving ? 'isloading' : '', title: (this.isRemoving) ? '' : `${this.langTasks.removeJobTip}` }, this.isRemoving, !this.isRemoving ?
                        index.h("i", { class: "download-icon", onClick: () => this.startRemoveJob() }, index.h("calcite-icon", { scale: "s", icon: "trash" }))
                        :
                            index.h("i", { class: "download-icon spinner" }, index.h("calcite-loader", { label: "", inline: "true" })))
                    : null) : '')) : '', this.job && this.showDetail == 'done' ?
            index.h("div", { class: "job-detail" }, index.h("detail-table", { job: this.job }))
            : ''), index.h("slot", { key: 'ed47d7ca4adb26331501bab1de55a56623d490f5' })));
    }
    get host() { return index.getElement(this); }
};
ReportTaskInfo.style = TaskInfoStyle0;

exports.detail_table = ReportDetailTable;
exports.report_icon = ReportIcon;
exports.task_info = ReportTaskInfo;

//# sourceMappingURL=detail-table_3.cjs.entry.js.map