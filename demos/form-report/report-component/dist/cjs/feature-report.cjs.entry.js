...
function fetchToken(url, requestOptions) {
    ...
    return util_service.request(url, options).then((response) => {
...
function isFederated(owningSystemUrl, portalUrl) {
    const normalizedPortalUrl = util_service.cleanUrl(normalizeOnlinePortalUrl(portalUrl)).replace(/https?:\/\//, "");
    const normalizedOwningSystemUrl = util_service.cleanUrl(owningSystemUrl).replace(/https?:\/\//, "");
...
function validateAppAccess(token, clientId, portal = "https://www.arcgis.com/sharing/rest") {
    ...
    return util_service.request(url, ro);
}
...
function revokeToken(requestOptions) {
    const url = `${util_service.cleanUrl(requestOptions.portal || "https://www.arcgis.com/sharing/rest")}/oauth2/revokeToken/`;
    ...
    return util_service.request(url, options).then((response) => {
        if (!response.success) {
            throw new util_service.ArcGISRequestError("Unable to revoke token", 500, response, url, options);
        }
        return response;
    });
}
...
class ArcGISIdentityManager {
    constructor(options) {
        ...
        this.portal = options.portal
            ? util_service.cleanUrl(options.portal)
    ...
    static beginOAuth2(options, win) {
        ...
        let authorizeUrl = `${util_service.cleanUrl(portal)}/oauth2/authorize`;
        ...
        if (provider !== "arcgis") {
            authorizeUrl = `${util_service.cleanUrl(portal)}/oauth2/social/authorize`;
        ...
        return setupAuth.then(() => {
            ...
            authorizeUrl = `${authorizeUrl}?${util_service.encodeQueryString(authorizeUrlParams)}`;
            // append additional params passed by the user
            if (params) {
                authorizeUrl = `${authorizeUrl}&${util_service.encodeQueryString(params)}`;
            }
            if (popup) {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    win.addEventListener(`arcgis-rest-js-popup-auth-${clientId}`, (e) => {
                        ...
                        if (e.detail.errorMessage) {
                            const error = new util_service.ArcGISAuthError(e.detail.errorMessage, e.detail.error);
                            reject(error);
                            return error;
                        }
    ...
    static completeOAuth2(options, win) {
        ...
        function reportError(errorMessage, error, originalUrl) {
            ...
            return Promise.reject(new util_service.ArcGISAuthError(errorMessage, error));
        }
         ...
         */
        if (pkce && params.code) {
            const tokenEndpoint = util_service.cleanUrl(`${portal}/oauth2/token/`);
            const codeVerifierStorageKey = `ARCGIS_REST_JS_CODE_VERIFIER_${clientId}`;
    ...
    static authorize(options, response) {
        ...
        const url = `${portal}/oauth2/authorize?${util_service.encodeQueryString(queryParams)}`;
    ...
    static fromCredential(credential, serverInfo) {
        ...
        return new ArcGISIdentityManager({
            portal: util_service.cleanUrl(credential.server.includes("sharing/rest")
                ? credential.server
    ...
    getUser(requestOptions) {
        if (this._pendingUserRequest) {
            ...
            this._pendingUserRequest = util_service.request(url, options).then((response) => {
                this._user = response;
    ...
    getPortal(requestOptions) {
        if (this._pendingPortalRequest) {
            ...
            const options = Object.assign(Object.assign({ httpMethod: "GET", authentication: this }, requestOptions), { rawResponse: false });
            this._pendingPortalRequest = util_service.request(url, options).then((response) => {
    ...
    getServerRootUrl(url) {
        const [root] = util_service.cleanUrl(url).split(/\/rest(\/admin)?\/services(?:\/|#|\?|$)/);
    ...
    getTokenForServer(url, requestOptions) {
        ...
        this._pendingTokenRequests[root] = this.fetchAuthorizedDomains().then(() => {
            return util_service.request(`${root}/rest/info`, {
                ...
                if (serverInfo.owningSystemUrl) {
                    ...
                    if (!isFederated(serverInfo.owningSystemUrl, this.portal)) {
                         ...
                         */
                        return util_service.request(`${serverInfo.owningSystemUrl}/sharing/rest/info`, requestOptions);
                    }
                }
    ...
    generateTokenForServer(tokenServicesUrl, serverUrl) {
        return util_service.request(tokenServicesUrl, {
            params: {
                token: this.token,
                serverUrl,
            ...
            referer: this.referer
                ...
                : typeof window !== "undefined" &&
                    ...
                    : /* istanbul ignore next */
                        util_service.NODEJS_DEFAULT_REFERER_HEADER
        };
        return (this.server
            ? util_service.request(`${this.getServerRootUrl(this.server)}/rest/info`).then((response) => {
                return util_service.request(response.authInfo.tokenServicesUrl, Object.assign({ params }, requestOptions));
            })
            : util_service.request(`${this.portal}/generateToken`, Object.assign({ params }, requestOptions)))
            .then((response) => {
...
const FeatureReport = class {
    constructor(hostRef) {
        ...
        this.userInfoGetted = index.createEvent(this, "userInfoGetted", 7);
        this.i18nStringUpdated = index.createEvent(this, "i18nStringUpdated", 7);
        this._credentialGetted = index.createEvent(this, "_credentialGetted", 7);
    ...
    templateItemIdChanged(newVal) {
        translate_service.PropsService.setProps({ templateItemId: newVal });
    }
    queryParametersChanged(newVal) {
        translate_service.PropsService.setProps({
    ...
    outputFormatChanged(newVal) {
        translate_service.PropsService.setProps({ outputFormat: newVal });
        this.outputFormat = translate_service.PropsService.outputFormat;
    }
    localeChanged(newVal) {
        this.localeChangeHandler(newVal);
        util_service.UtilService.getService().setDir();
    }
    showChanged(newValue) {
        // console.log(newValue, oldValue);
        translate_service.PropsService.setProps({ show: newValue });
    ...
    hideChanged(newVal) {
        translate_service.PropsService.setProps({ hide: newVal });
        this.visibleConf = this.generateVisibleElems();
        this.reportService.setHelperObj({
            visibleConf: this.visibleConf
        });
    ...
    inputFeatureTemplateChanged(newVal) {
        translate_service.PropsService.setProps({ inputFeatureTemplate: newVal });
        this.stateService.notifyDataChanged('update-features-preview', { value: this.inputFeatureTemplate });
    }
    labelChanged(newVal) {
        translate_service.PropsService.setProps({ label: newVal });
    ...
    componentWillLoad() {
        translate_service.PropsService.setProps(this);
        this.portalUrl = translate_service.PropsService.portalUrl;
        this.outputFormat = translate_service.PropsService.outputFormat;
        return Promise.resolve(true)
            .then(() => {
            return translate_service.TranslateService.getService().getTranslate();
            ...
            this.langTasks = langTasks;
            this.langCommon = res.common;
            this.i18nStringUpdated.emit({ locale: translate_service.PropsService.locale, i18n: this.langCommon });
            ...
            if (this.token) {
                ...
                if (this.isOAuthCallbackpage()) {
                    ...
                    return util_service.request(requestUrl, param).then((response) => {
    ...
    init() {
        ...
        this.stateService.subscribe('job-complete', (job) => {
            ...
            if (idx >= 0) {
                if (job.jobStatus === report_service.EsriJobStatusType.partialSucceeded || job.jobStatus === report_service.EsriJobStatusType.succeeded) {
        ...
        return Promise.resolve(true)
            ...
            if (!this.locale) {
                const newLocale = translate_service.PropsService.getLocale({
                    userInfo: portalInfo.user,
                    portalInfo: portalInfo
                });
                if (newLocale !== translate_service.PropsService.locale) {
            ...
            if (translate_service.PropsService.surveyItemId) {
                return utilService.getSurveyItemInfo(translate_service.PropsService.surveyItemId);
            }
            // return true;
        })
            .catch((e) => {
    ...
    localeChangeHandler(newLocale) {
        translate_service.PropsService.setProps({ locale: newLocale });
        return Promise.resolve(true)
            .then(() => {
            return translate_service.TranslateService.getService().getTranslate();
        })
    ...
    render() {
        var _a, _b, _c, _d, _e;
        return (index.h(index.Host, { key: '372606ed791cd7d1ef7fb882c716ca76840a45f2' }, index.h("calcite-panel", { key: 'a7346f03688cfdb7440bf8e40363176fa1c8515b' }, index.h("div", { key: '652e818cf3cf99a5df2a0a19e0c141c96acd080d', style: { display: (this.state === 'generate-report' && this.token) ? 'block' : 'none' } }, this.visibleConf.indexOf('inputFeatures') < 0 ? '' : index.h("features-preview", { queryParameters: this.queryParameters, inputFeatureTemplate: this.inputFeatureTemplate }), this.visibleConf.indexOf('selectTemplate') < 0 || !this.surveyItemInfo ? '' : index.h("template-chooser", { selectedTemplateId: this.templateItemId, templateIds: this.reportTemplateIds }), this.visibleConf.indexOf('reportSetting') < 0 ? '' : index.h("report-settings", { visibleElems: this.visibleConf, mergeFiles: this.mergeFiles, outputFormat: this.outputFormat, fileName: this.outputReportName }), index.h("report-generator", { key: 'c5d2c57bd3086446b9317ef344c5ae86a9f11420', visibleConf: this.visibleConf, templateItemId: this.templateItemId, onReportCreated: (evt) => { this.generateReportHander(evt); } }), this.visibleConf.indexOf('recentReports') < 0 ?
            null :
            index.h("div", { class: "banner" }, index.h("calcite-action", { onClick: () => this.switchState('report-list'), icon: "chevrons-right", "text-enabled": true }, index.h("span", null, (_a = this.langTasks) === null || _a === void 0 ? void 0 : _a.label), ((_b = this.checkingList) === null || _b === void 0 ? void 0 : _b.length) ?
                ...
                : null))), this.token ?
            index.h("task-list", { style: { display: (this.state === 'report-list') ? 'block' : 'none' }, jobs: this.jobs, onGoBackClicked: () => { this.state = 'generate-report'; } })
            : '', this.error ?
            index.h("calcite-alert", { slot: "alerts", open: true, onCalciteAlertClose: () => { this.error = null; }, label: this.error.html, icon: true, kind: this.error.alertType || 'danger', placement: "top", scale: "s" }, index.h("div", { class: "error-message", slot: "message" }, this.error.html, this.error.detail ?
                index.h("p", { innerHTML: this.error.detail })
                : null))
            : null, this.state === 'login' && !translate_service.PropsService.token ?
            index.h("calcite-modal", { "aria-labelledby": "modal-title", id: "login-modal", "outside-close-disabled": "true", scale: "s", "width-scale": "s", "close-button-disabled": "true", open: "true" }, index.h("div", { slot: "header", id: "modal-title" }, (_c = this.langCommon) === null || _c === void 0 ? void 0 : _c.signIn), index.h("div", { slot: "content" }, index.h("calcite-label", { style: { height: '100px' } }, index.h("p", null, (_d = this.langCommon) === null || _d === void 0 ? void 0 : _d.signInMsg), index.h("calcite-button", { id: "reportLoginBtn", kind: "brand", onClick: () => this.startLogin() }, (_e = this.langCommon) === null || _e === void 0 ? void 0 : _e.signIn))))
            : ''), index.h("slot", { key: '22b3f6908b27655eabd64ab0e452f706d4528752' })));
    }
...
exports.feature_report = FeatureReport;

//# sourceMappingURL=feature-report.cjs.entry.js.map