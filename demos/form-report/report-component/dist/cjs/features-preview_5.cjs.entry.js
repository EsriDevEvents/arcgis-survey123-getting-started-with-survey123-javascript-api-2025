...
const report_service = require('./report.service-5072c121.js');
...
const getUserContent = (requestOptions) => {
    ...
    return determineOwner(requestOptions)
        .then((owner) => `${util_service.getPortalUrl(requestOptions)}/content/users/${owner}${suffix}`)
        .then((url) => util_service.request(url, {
        httpMethod: "GET",
        authentication,
        params: {
            start,
...
const FeaturesPreview = class {
    constructor(hostRef) {
        ...
        this.reportService = report_service.ReportService.getService();
    ...
    init() {
        ...
        return this.getDisplayFields()
            ...
            const params = Object.assign({
                ...
                authentication: translate_service.PropsService.token, // todo: pass the portalUrl
                outFields: outFields,
            ...
            return report_service.queryFeatures(params);
        })
            .then((result) => {
            // no feature
    ...
    render() {
        var _a, _b;
        return (index.h(index.Host, { key: 'f2c77b92f32d439574043b1746bf8b4f36df78e9' }, index.h("div", { key: '250a3421f00f699028066cddbb133be62f3df570' }, index.h("div", { key: '4e2c1ac52c1bd185cfba5df6ab87155a3762615c', class: "heading" }, (_b = (_a = this.translator) === null || _a === void 0 ? void 0 : _a.customPrint) === null || _b === void 0 ? void 0 :
            _b.inputFeatures, " (", this.featureCount, ")"), index.h("div", { key: 'ad8ebea8558d0ec9d5e25b3b97ee2b3e070b6d11', innerHTML: this.featureStr })), index.h("slot", { key: '4346d0030a306b4ae30ec8b808667147e0ffc080' })));
    }
};
...
const ReportGenerator = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.reportCreated = index.createEvent(this, "reportCreated", 7);
        this.batchPrintLimitCount = 2000;
        // private buttonClickable: boolean;
        this.checkingList = [];
        this.reportService = report_service.ReportService.getService();
        this.utilService = util_service.UtilService.getService();
        this.state = state_service.StateService.getService();
    ...
    estimateCredits() {
        ...
        if (!(this.creditsInfo && this.creditStatus === 'estimating')) {
            ...
            const templateItemId = paramStore.templateItemId || translate_service.PropsService.templateItemId;
            const params = {
                queryParameters: translate_service.PropsService.queryParameters,
                featureLayerUrl: translate_service.PropsService.featureLayerUrl,
                templateItemId: templateItemId,
                surveyItemId: translate_service.PropsService.surveyItemId
            };
    ...
    executeCustomPrint(isTestMode) {
        ...
        if (isTestMode) {
            const params = {
                queryParameters: translate_service.PropsService.queryParameters,
                templateItemId: paramStore.templateItemId || translate_service.PropsService.templateItemId,
                surveyItemId: translate_service.PropsService.surveyItemId,
                featureLayerUrl: translate_service.PropsService.featureLayerUrl,
                outputReportName: paramStore.outputPackageName || translate_service.PropsService.outputPackageName || paramStore.outputReportName || translate_service.PropsService.outputReportName || '' // !this.saveToItem ? fName + '' : '',
            };
            ...
            const mergeFiles = paramStore.mergeFiles || translate_service.PropsService.mergeFiles;
        ...
        let fileName = paramStore.outputPackageName || translate_service.PropsService.outputPackageName || paramStore.outputReportName || translate_service.PropsService.outputReportName || '';
        ...
        return Promise.resolve()
            ...
            let uploadInfo = null;
            // if (this.saveToItem) {
            uploadInfo = paramStore.uploadInfo || translate_service.PropsService.uploadInfo || {
            ...
            // fileName = null;
            // }
            const packageFiles = translate_service.PropsService.packageFiles + '' === 'true' ? true : (translate_service.PropsService.packageFiles + '' === 'false' ? false : 'auto');
            const params = {
                queryParameters: translate_service.PropsService.queryParameters, //JSON.stringify(queryParams),
                templateItemId: paramStore.templateItemId || translate_service.PropsService.templateItemId,
            ...
            if (translate_service.PropsService.surveyItemId) {
                params.surveyItemId = paramStore.surveyItemId || translate_service.PropsService.surveyItemId;
            }
            if (paramStore.outputFormat || translate_service.PropsService.outputFormat) {
                params.outputFormat = paramStore.outputFormat || translate_service.PropsService.outputFormat;
            }
            if (translate_service.PropsService.webmapItemId) {
                params.webmapItemId = translate_service.PropsService.webmapItemId;
            }
            if (Number(translate_service.PropsService.mapScale) || Number(translate_service.PropsService.mapScale) === 0) {
                params.mapScale = Number(translate_service.PropsService.mapScale);
            }
            if (translate_service.PropsService.locale) {
                params.locale = '||' + translate_service.PropsService.locale;
            }
            if (translate_service.PropsService.utcOffset) {
                params.utcOffset = translate_service.PropsService.utcOffset;
            }
            // Use outputReportName as outputPackageName if it doesn't have any variables and outputPackageName is not specified by user
            // https://devtopia.esri.com/Beijing-R-D-Center/feature-report/issues/252
            const outputPackageName = paramStore.outputPackageName || translate_service.PropsService.outputPackageName;
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'aca32173e6e0e969c0321fdd6dc10145da134760' }, index.h("p", { key: '2a893d489dfe3a83bb27a2fe590621223025894d' }, this.supportShowCredits), this.supportShowCredits && this.visibleConf.indexOf('showCredits') >= 0 ?
            index.h("div", { class: "credits-info" }, index.h("div", { class: "show-credits has-spinner" }, this.creditsInfo && this.creditStatus === 'estimating' ?
                index.h(index.Fragment, null, index.h("span", { class: `${this.featureCount < 1 || (!this.printTemplates.length) ? 'disabled' : ''}`, onClick: () => this.estimateCredits() }, `${(_a = this.translator) === null || _a === void 0 ? void 0 : _a.common.calculating}`), index.h("calcite-loader", { label: (_b = this.translator) === null || _b === void 0 ? void 0 : _b.common.calculating, inline: "true" }))
                :
                    index.h("calcite-link", { disabled: `${this.featureCount < 1 || (!this.printTemplates.length) ? true : false}`, onClick: () => this.estimateCredits() }, `${(_d = (_c = this.translator) === null || _c === void 0 ? void 0 : _c.customPrint) === null || _d === void 0 ? void 0 : _d.creditsEstimate}`)), this.creditsInfo && this.creditStatus === 'finished' && this.featureCount > 0 && this.printTemplates.length ?
                index.h("div", { class: "credits-result-info" }, index.h("span", null, (_f = (_e = this.translator) === null || _e === void 0 ? void 0 : _e.customPrint) === null || _f === void 0 ? void 0 : _f.creditsRecordsCount.replace('${$recordsCount}', `${this.featureCount}`).replace('${$requiredCredits}', `${this.creditsInfo.cost}`))) : '', this.creditsInfo && this.creditStatus === 'changed' && this.featureCount > 0 && this.printTemplates.length ?
                index.h("div", { class: "credits-result-info" }, index.h("span", null, (_h = (_g = this.translator) === null || _g === void 0 ? void 0 : _g.customPrint) === null || _h === void 0 ? void 0 : _h.credtisResultInvalid)) : '') : '', index.h("div", { key: 'bc95469bad71ab752a604a34b258471f73992a8f', class: `execute-btn ${!this.isPortal && this.featureCount <= this.batchPrintLimitCount && this.canShowEstimateCredits ? '' : 'portal-btn'}` }, index.h("a", { key: 'cdc19e375ccf3d035b72706b70ba771b185e9706', class: "no-hover" }, index.h("calcite-button", { key: '32ec0cfa2757e0d8124ba0c9703d90f2d1850e62', id: "submit-btn", disabled: this.isPrinting || !this.buttonClickable(), onClick: () => this.executeCustomPrint() }, (_k = (_j = this.translator) === null || _j === void 0 ? void 0 : _j.customPrint) === null || _k === void 0 ? void 0 :
            _k.generate, this.isPrinting ?
            index.h("calcite-loader", { label: (_m = (_l = this.translator) === null || _l === void 0 ? void 0 : _l.common) === null || _m === void 0 ? void 0 : _m.loading, inline: "true" })
            ...
            null :
            index.h("calcite-notice", { open: true, kind: "danger", scale: "s", width: "auto" }, index.h("div", { slot: "message" }, (_q = (_p = this.translator) === null || _p === void 0 ? void 0 : _p.customPrint) === null || _q === void 0 ? void 0 : _q.chooseTemplateNoOneYet1)))), index.h("slot", { key: '5ff5f48aaf6df90e383672eb15e850a46a867f48' })));
    }
};
...
const ReportSettings = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.fileOptionChange = index.createEvent(this, "fileOptionChange", 7);
        this.fileNameChange = index.createEvent(this, "fileNameChange", 7);
        this.selectedFolderChange = index.createEvent(this, "selectedFolderChange", 7);
        this.selectedFileFormatChange = index.createEvent(this, "selectedFileFormatChange", 7);
        this.reportService = report_service.ReportService.getService();
        this.state = state_service.StateService.getService();
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'f7533ed880148f57d8d02b32f1c098b7a50aaf0c' }, index.h("div", { key: '2a9949c62bbb802797eb7f959769290f665d5d58', class: "heading" }, (_b = (_a = this.translator) === null || _a === void 0 ? void 0 : _a.customPrint) === null || _b === void 0 ? void 0 : _b.resultSettingsLabel1), this.visibleElems.indexOf('fileOptions') < 0 ?
            null : index.h("file-option-chooser", { fileOption: this.mergeFiles, onFileOptionChange: (evt) => { this.fileOptChangeHandler(evt); } }), this.visibleElems.indexOf('reportName') < 0 ?
            null :
            index.h("calcite-label", null, (_d = (_c = this.translator) === null || _c === void 0 ? void 0 : _c.customPrint) === null || _d === void 0 ? void 0 :
                _d.saveToAGOItemName, index.h("calcite-input-text", { placeholder: (_f = (_e = this.translator) === null || _e === void 0 ? void 0 : _e.customPrint) === null || _f === void 0 ? void 0 : _f.saveToAGOItemName, value: this.fileName, onCalciteInputTextChange: (evt) => this.fileNameChangeHandler(evt) })), this.visibleElems.indexOf('saveToAGSAccount') < 0 ? null :
            index.h("calcite-label", null, index.h("div", { class: "folder-label" }, index.h("span", null, (_h = (_g = this.translator) === null || _g === void 0 ? void 0 : _g.customPrint) === null || _h === void 0 ? void 0 : _h.saveToAGOFolder), index.h("calcite-dropdown", { width: "l", scale: "m", "close-on-select-disabled": true, type: "click", "width-scale": "l" }, index.h("calcite-button", { slot: "trigger", "icon-start": "gear", appearance: "transparent" }), index.h("calcite-dropdown-group", null, index.h("div", { class: "conflict-name-setting" }, index.h("calcite-radio-button-group", { name: "conflict-name-options", layout: "vertical" }, index.h("calcite-label", { layout: "inline" }, (_k = (_j = this.translator) === null || _j === void 0 ? void 0 : _j.customPrint) === null || _k === void 0 ? void 0 :
                _k.nameConflictOptLabel, index.h("calcite-icon", { id: "info-icon", icon: "question", scale: "s" }, " "), index.h("calcite-tooltip", { label: (_m = (_l = this.translator) === null || _l === void 0 ? void 0 : _l.customPrint) === null || _m === void 0 ? void 0 : _m.nameConflictOptLabel, "reference-element": "info-icon" }, index.h("span", null, (_p = (_o = this.translator) === null || _o === void 0 ? void 0 : _o.customPrint) === null || _p === void 0 ? void 0 : _p.nameConflictOptLabel))), index.h("calcite-label", { layout: "inline" }, index.h("calcite-radio-button", { value: "rename", checked: this.conflictBehavior == 'rename', onCalciteRadioButtonChange: (evt) => this.changeConflictBehavior(evt) }), (_r = (_q = this.translator) === null || _q === void 0 ? void 0 : _q.customPrint) === null || _r === void 0 ? void 0 :
                _r.nameConflictOptRename), index.h("calcite-label", { layout: "inline" }, index.h("calcite-radio-button", { value: "replace", checked: this.conflictBehavior == 'replace', onCalciteRadioButtonChange: (evt) => this.changeConflictBehavior(evt) }), (_t = (_s = this.translator) === null || _s === void 0 ? void 0 : _s.customPrint) === null || _t === void 0 ? void 0 :
                _t.nameConflictOptReplace), index.h("calcite-label", { layout: "inline" }, index.h("calcite-radio-button", { value: "fail", checked: this.conflictBehavior == 'fail', onCalciteRadioButtonChange: (evt) => this.changeConflictBehavior(evt) }), (_v = (_u = this.translator) === null || _u === void 0 ? void 0 : _u.customPrint) === null || _v === void 0 ? void 0 :
                _v.nameConflictOptSkip)))))), index.h("calcite-tooltip", { label: this.helperObj.canCreateItem === false ? this.langReport.saveToAGONoCreatePrivilegeTip : '', "reference-element": "folder-selector" }, index.h("span", null, this.helperObj.canCreateItem === false ? this.langReport.saveToAGONoCreatePrivilegeTip : '')), index.h("calcite-combobox", { id: "folder-selector", disabled: this.helperObj.canCreateItem === false, placeholder: (_x = (_w = this.translator) === null || _w === void 0 ? void 0 : _w.customPrint) === null || _x === void 0 ? void 0 : _x.saveToAGOFolder, "selection-mode": "single-persist", clearDisabled: "true", maxItems: "7", value: this.selectedFolder, onCalciteComboboxChange: () => { this.selectedFolderChangeHandler(); } }, (this.folders || []).map((folder) => {
                var _a;
                return index.h("calcite-combobox-item", { value: folder.value, "text-label": folder.label, selected: folder.value === ((_a = this.selectedFolder) === null || _a === void 0 ? void 0 : _a.id) });
            }))), this.visibleElems.indexOf('outputFormat') < 0 ? null :
            index.h("calcite-label", null, (_z = (_y = this.translator) === null || _y === void 0 ? void 0 : _y.customPrint) === null || _z === void 0 ? void 0 :
                _z.outputFormat, index.h("calcite-combobox", { placeholder: (_1 = (_0 = this.translator) === null || _0 === void 0 ? void 0 : _0.customPrint) === null || _1 === void 0 ? void 0 : _1.outputFormat, "selection-mode": "single-persist", clearDisabled: "true", value: this.outputFormat, onCalciteComboboxChange: (evt) => this.outputFormatChangeHandler(evt) }, index.h("calcite-combobox-item", { value: 'docx', selected: this.outputFormat === 'docx', "text-label": (_3 = (_2 = this.translator) === null || _2 === void 0 ? void 0 : _2.customPrint) === null || _3 === void 0 ? void 0 : _3.outputFormatDocx }), index.h("calcite-combobox-item", { value: 'pdf', selected: this.outputFormat === 'pdf', "text-label": (_5 = (_4 = this.translator) === null || _4 === void 0 ? void 0 : _4.customPrint) === null || _5 === void 0 ? void 0 : _5.outputFormatPdf }))), index.h("slot", { key: '0ea7f1954b4d990870bce17a6f6ab3d9904242df' })));
    }
...
const ReportTaskList = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.goBackClicked = index.createEvent(this, "goBackClicked", 7);
    ...
    render() {
        var _a, _b;
        return (index.h(index.Host, { key: '9e04f7f484918b584487a2cd887a4473424e2a86' }, index.h("div", { key: '7bd7270ce284bdc0d8bf858c6d4726e165d96b88', class: "banner" }, index.h("calcite-action", { key: '53d1f3665321ff931bfd32771d01baba7995676c', onClick: () => this.goBack(), text: (_a = this.langTasks) === null || _a === void 0 ? void 0 : _a.label, icon: "chevrons-left", "text-enabled": true })), this.state === 'ready' && this.langReport && this.langCommon && this.langTasks && this.jobList ?
            index.h("div", { class: "list" }, this.jobs.length < 1 ?
                index.h("div", { class: "no-tasks" }, this.langTasks.noTaskDesc)
                : '', this.jobs.length ?
                index.h(index.Fragment, null, index.h("div", { class: "no-tasks" }, this.langTasks.limitationDesc.replace('${$maxJobCount}', `${this.recentTasksCount}`)), index.h("ul", null, this.jobs.map((job, index$1) => {
                    return index.h("li", { key: 'li_' + job.jobId }, this.jobSplitDate(job, index$1) ? index.h("p", { class: "job-date" }, this.jobSplitDate(job, index$1)) : '', index.h("task-info", { key: job.jobId, job: job, onJobRemoved: (evt) => this.removeJob(evt) }));
                }))) : '')
            :
                index.h("div", { class: "loading" }, index.h("calcite-loader", { label: (_b = this.langCommon) === null || _b === void 0 ? void 0 : _b.loading })), index.h("slot", { key: '6e879c58da983aa00e6040ae7a5e7efbf61d8a2b' })));
    }
};
...
const TemplateChooser = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.selectedTemplateChange = index.createEvent(this, "selectedTemplateChange", 7);
        this.reportService = report_service.ReportService.getService();
        this.stateService = state_service.StateService.getService();
        this.langObj = undefined;
    ...
    init() {
        const globalProps = translate_service.PropsService;
        const reportService = report_service.ReportService.getService();
    ...
    render() {
        var _a, _b, _c;
        return (index.h(index.Host, { key: '7f3ef2c784e04f8a1a0d1861200d4c794ed50fb7' }, index.h("div", { key: '5e54734af22f84f41e1f5a6c87773e6c0a302869', class: "heading" }, (_a = this.translator) === null || _a === void 0 ? void 0 : _a.customPrint.chooseTemplateLabel1), index.h("calcite-combobox", { key: '7eb5e61f8d97e61b6421ac73a6a814842e5e11a0', placeholder: (_b = this.translator) === null || _b === void 0 ? void 0 : _b.customPrint.chooseTemplateLabel1, "selection-mode": "single-persist", clearDisabled: "true", value: this.selectedTemplateId, onCalciteComboboxChange: (evt) => { this.selectedTemplateChangeHandler(evt); } }, (this.templates || []).map((template) => {
            return index.h("calcite-combobox-item", { value: template.id, "text-label": template.title, selected: template.id === this.selectedTemplateId });
        })), (this.templates || []).length > 0 ?
            null :
            index.h("calcite-notice", { open: true, kind: "danger", scale: "s", width: "auto" }, index.h("div", { slot: "message" }, (_c = this.translator) === null || _c === void 0 ? void 0 : _c.customPrint.chooseTemplateNoOneYet1)), index.h("slot", { key: '2cea7b7e4e0d4591c35e8466f4295af2b6119c66' })));
    }
...
exports.features_preview = FeaturesPreview;
exports.report_generator = ReportGenerator;
exports.report_settings = ReportSettings;
exports.task_list = ReportTaskList;
exports.template_chooser = TemplateChooser;

//# sourceMappingURL=features-preview_5.cjs.entry.js.map